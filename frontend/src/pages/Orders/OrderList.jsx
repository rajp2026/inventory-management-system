...
import { getOrders, getOrderCount, deleteOrder } from '../../api/orderApi';
  ...
  const columns = [
    {
      ...
      label: 'Order ID',
      render: (row) => (
        <Link to={`/orders/${row.id}`} className="text-indigo-600 hover:text-indigo-900 font-medium transition-colors">
          {row.id.slice(0, 8)}...
        </Link>
      )
    },
    {
      ...
      label: 'Customer',
      render: (row) => (
        <span className="text-gray-800 font-medium">{row.customer_name || 'N/A'}</span>
      )
    },
    {
      key: 'order_items',
      ...
      render: (row) => (
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
          {row.order_items?.length || 0} item{(row.order_items?.length || 0) !== 1 ? 's' : ''}
        </span>
      )
    },
    {
      ...
      label: 'Total',
      render: (row) => (
        <span className="text-gray-900 font-semibold">{formatCurrency(row.total_amount)}</span>
      )
    },
    {
      key: 'actions',
      ...
      render: (row) => (
        <div className="flex gap-3">
          <Link to={`/orders/${row.id}`} className="text-indigo-600 hover:text-indigo-900 font-medium transition-colors">View</Link>
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Total orders: {totalCount}</p>
        </div>
        <Link
          to="/orders/create"
      ...
      {orders.length === 0 && !loading ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <p className="text-gray-500 text-lg">No orders found. Start by creating one!</p>
        </div>
      ) : (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <p>Are you sure you want to delete this order? Stock quantities will be restored. This action cannot be undone.</p>
        </div>
      </Modal>
