...
import { getCustomers, getCustomerCount, deleteCustomer } from '../../api/customerApi';
    ...
    { key: 'phone', label: 'Phone' },
    {
      ...
      render: (row) => (
        <div className="flex gap-3">
          <Link to={`/customers/${row.id}/edit`} className="text-indigo-600 hover:text-indigo-900 font-medium transition-colors">Edit</Link>
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Total customers: {totalCount}</p>
        </div>
        <Link 
          to="/customers/create"
      ...
      {customers.length === 0 && !loading ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <p className="text-gray-500 text-lg">No customers found. Start by adding one!</p>
        </div>
      ) : (
